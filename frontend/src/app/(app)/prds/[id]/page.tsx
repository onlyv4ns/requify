...
import { useRevisionHistory } from "@/components/RevisionHistory";
...
export default function PrdDetailPage() {
  ...
  const { dialog: historyDialog, open: openHistory } = useRevisionHistory(id, setPrd);
  ...
  async function handleDuplicate() {
    if (!prd || duplicating) return;
    setDuplicating(true);
    try {
      const copy = await duplicatePrd(id);
      router.push(`/prds/${copy.id}`);
    } catch (err) {
      setDuplicating(false);
      setError(err instanceof Error ? err.message : "Failed to duplicate PRD");
    }
  }

  async function handleShare() {
    if (!prd || sharing) return;
    setSharing(true);
    setShareError("");
    try {
      let token = prd.share_token;
      if (!token) {
        token = (await sharePrd(id)).share_token;
        setPrd({ ...prd, share_token: token });
      }
      await navigator.clipboard.writeText(`${window.location.origin}/shared/${token}`);
    } catch (err) {
      setShareError(err instanceof Error ? err.message : "Failed to share PRD");
    } finally {
      setSharing(false);
    }
  }
  ...
  async function handleUnshare() {
    ...
    try {
      await unsharePrd(id);
      setPrd({ ...prd, share_token: null });
    } catch (err) {
      setShareError(err instanceof Error ? err.message : "Failed to unshare PRD");
    } finally {
      setSharing(false);
    }
  }
      ...
      <div className="print-area">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex shrink-0 gap-2 print:hidden">
            ...
            <button
              onClick={() => openHistory(prd.content)}
              className="flex items-center gap-1.5 rounded border border-border px-3 py-1.5 text-xs font-medium text-foreground/80 hover:bg-accent-dim/40"
            >
            ...
            <button
              onClick={handleShare}
              disabled={sharing}
              title={prd.share_token ? "Copy share link" : "Create a public read-only link"}
              className="flex items-center gap-1.5 rounded border border-border px-3 py-1.5 text-xs font-medium text-foreground/80 hover:bg-accent-dim/40 disabled:opacity-50"
            >
              {prd.share_token ? <Link2 size={14} /> : <Share2 size={14} />}
              {sharing ? "Sharing..." : prd.share_token ? "Copy link" : "Share"}
            </button>
            {prd.share_token && (
              <button
        ...
        {shareError && <p className="mb-4 text-sm text-red-400">{shareError}</p>}
      ...
      <div className="mt-6 flex flex-col gap-2 print:hidden">
        ...
        <form onSubmit={handleAsk} className="flex items-end gap-2">
          <textarea
            ...
            placeholder="e.g. Would Supabase be a good fit here? (Enter to send, Shift+Enter for a new line)"
            rows={2}
